// $Id: instruction.c,v 1.61 2024/11/10 14:34:25 leavens Exp $
#include <errno.h>
#include <string.h>
#include "bof.h"
...
// Write the binary instruction bi to bf in binary,
// but exit with an error if there is a problem.
void instruction_write_bin_instr(BOFFILE bf, bin_instr_t bi)
{
    switch (instruction_type(bi)) {
    case comp_instr_type:
	instruction_write_compInstr(bf, bi.comp);
	break;
    case other_comp_instr_type:
	instruction_write_otherCompInstr(bf, bi.othc);
	break;
    case immed_instr_type:
	instruction_write_immedInstr(bf, bi.immed);
	break;
    case jump_instr_type:
	instruction_write_jumpInstr(bf, bi.jump);
	break;
    case syscall_instr_type:
	instruction_write_syscallInstr(bf, bi.syscall);
	break;
    default:
	bail_with_error("Unknown instruction type in instruction_write_bin_instr (%d)",
			instruction_type(bi));
	break;
    }
}
...
syscall_type instruction_token2SyscallCode(int toknum)
{
	...
	break;
    default:
	bail_with_error("Unknown token type in instruction_token2SyscallCode: %d",
			toknum);
